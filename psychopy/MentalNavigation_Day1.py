...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "welcomePg2" ---
    ...
    # --- Initialize components for Routine "welcomePg3" ---
    ...
    blockSize = 5 #15 # Block is collection of Trials. Number of trials in a block
    ...
    # --- Prepare to start Routine "welcomePg1" ---
    # create an object to store info about Routine welcomePg1
    welcomePg1 = data.Routine(
        name='welcomePg1',
        components=[welcomeText, keyWelcome],
    )
    welcomePg1.status = NOT_STARTED
    ...
    keyWelcome.rt = []
    _keyWelcome_allKeys = []
    # store start times for welcomePg1
    welcomePg1.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    welcomePg1.tStart = globalClock.getTime(format='float')
    welcomePg1.status = STARTED
    thisExp.addData('welcomePg1.started', welcomePg1.tStart)
    # keep track of which components have finished
    welcomePg1Components = welcomePg1.components
    for thisComponent in welcomePg1.components:
    ...
    # --- Run Routine "welcomePg1" ---
    welcomePg1.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            welcomePg1.forceEnded = routineForceEnded = True
            break
    ...
    # --- Ending Routine "welcomePg1" ---
    for thisComponent in welcomePg1.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for welcomePg1
    welcomePg1.tStop = globalClock.getTime(format='float')
    welcomePg1.tStopRefresh = tThisFlipGlobal
    thisExp.addData('welcomePg1.stopped', welcomePg1.tStop)
    thisExp.nextEntry()
    # the Routine "welcomePg1" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "welcomePg2" ---
    # create an object to store info about Routine welcomePg2
    welcomePg2 = data.Routine(
        name='welcomePg2',
        components=[welcomeText_2, keyWelcome_2],
    )
    welcomePg2.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # store start times for welcomePg2
    welcomePg2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    welcomePg2.tStart = globalClock.getTime(format='float')
    welcomePg2.status = STARTED
    thisExp.addData('welcomePg2.started', welcomePg2.tStart)
    welcomePg2.maxDuration = None
    # keep track of which components have finished
    welcomePg2Components = welcomePg2.components
    for thisComponent in welcomePg2.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "welcomePg2" ---
    welcomePg2.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            welcomePg2.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in welcomePg2.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "welcomePg2" ---
    for thisComponent in welcomePg2.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for welcomePg2
    welcomePg2.tStop = globalClock.getTime(format='float')
    welcomePg2.tStopRefresh = tThisFlipGlobal
    thisExp.addData('welcomePg2.stopped', welcomePg2.tStop)
    thisExp.nextEntry()
    # the Routine "welcomePg2" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "welcomePg3" ---
    # create an object to store info about Routine welcomePg3
    welcomePg3 = data.Routine(
        name='welcomePg3',
        components=[welcomeText_3, keyWelcome_3],
    )
    welcomePg3.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # store start times for welcomePg3
    welcomePg3.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    welcomePg3.tStart = globalClock.getTime(format='float')
    welcomePg3.status = STARTED
    thisExp.addData('welcomePg3.started', welcomePg3.tStart)
    welcomePg3.maxDuration = None
    # keep track of which components have finished
    welcomePg3Components = welcomePg3.components
    for thisComponent in welcomePg3.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "welcomePg3" ---
    welcomePg3.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            welcomePg3.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in welcomePg3.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "welcomePg3" ---
    for thisComponent in welcomePg3.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for welcomePg3
    welcomePg3.tStop = globalClock.getTime(format='float')
    welcomePg3.tStopRefresh = tThisFlipGlobal
    thisExp.addData('welcomePg3.stopped', welcomePg3.tStop)
    thisExp.nextEntry()
    # the Routine "welcomePg3" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    distances1 = [random.choice(allDistances) for _ in range(blockSize*3)]
    distances2 =  [random.choice(allDistances) for  _ in range(1)]
    ...
    visualModeOptions = [True,False,True]
