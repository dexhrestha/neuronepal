...
async function main() {
    ...
    exp.createTrialSequence([
      new Block({
        variables: {
          ...
          speed:[...Array.from({ length:config.numTrials}, (_, i) => exp.cfg.speed[0]),...Array.from({ length:config.numTrials}, (_, i) => exp.cfg.speed[1]),...Array.from({ length:config.numTrials}, (_, i) => exp.cfg.speed[2])]
        },
        options: {
  ...
  exp.state.init(
    [
      ...
      '-', // dash
      'DOWN',
      'REPRODUCE',// INSTRUCTION FOR REPRODUCE
      'RETURN',
      'FINISH',
      'ADVANCE',
      'SURVEY',
    ...
    switch (exp.state.current) {
      ...
      case '-':
        if (!cursor.atHome){
          ...
          if(exp.cfg.sequence.length>trial.currentGap){
              if(exp.state.expired(exp.cfg.sequence[trial.currentGap]/trial.speed)){
                trial.currentGap+=1;
                exp.state.next('.')
              }
          }else{   
