...
async function main() {
  ...
  let moveLeft =false;
  let moveRight = false;
  const landmarkHeight = 0.4;
  const landmarkWidth = 0.4;
  const interlmdistance = 0.4;
  const landmarks = [];
  const textureLoader = new TextureLoader();
  const imageset = 1;
  const images = [...Array(9).keys()].map(num => \`landmarks/images/imageset\${imageset}/\${num}.jpg\`)
...
exp.sceneManager.scene.add(mask,mask2,mask3)
   ...
   */
  const stepSizes = new Array(0.03, 0.015, 0.06)

  exp.createTrialSequence([
    new Block({
      variables: {
        
        startId : Array.from({ length: 72 * 3 }, (_, i) => Math.floor(i / 8)%9),
        // startId : Array.from({ length:  5 }, (_, i) => 0),
        targetId :  Array.from({ length: 9*3  }, (_, i) => [...Array(9).keys()].filter(num => num !== i % 9)).flat(),
        // targetId :  Array.from({ length:5}, (_, i) => 8),
        stepSize :[
          ...Array(72).fill(stepSizes[0]),
          ...Array(72).fill(stepSizes[1]),
          ...Array(72).fill(stepSizes[2])
        ]
  ...
  function calcFunc() {
    ...
    target.reached =  target.distance < 0.1;
  }
