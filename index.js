...
async function main() {
  ...
  exp.instructions = new InstructionsPanel({
    content: `Use the keyboard to reproduce the morse code.`,
  });
  ...
  function stateFunc() {
    ...
    switch (exp.state.current) {
      ...
      case 'SETUP':
        ...
        trial.speed = getRandomItem(exp.cfg.speed)
      ...
      case 'DELAY':
        if (!cursor.atHome) {
          ...
          exp.state.next(exp.cfg.sequence[trial.currentToken]===' '?'GAP':exp.cfg.sequence[trial.currentToken])
        }
        break;
      ...
      case '.':
        if (!cursor.atHome){
          exp.state.next('START');
        }else if (exp.state.expired(exp.cfg.dotDuration*trial.speed)){
          exp.state.next(exp.cfg.sequence[++trial.currentToken]===' '?'GAP':exp.cfg.sequence[trial.currentToken])
        }
      break;
      ...
      case 'GAP':
        if (!cursor.atHome){
          exp.state.next('START');
        }else if (exp.state.expired(exp.cfg.gapDuration*trial.speed)){
          console.log(exp.cfg.sequence[trial.currentToken])
          exp.state.next(exp.cfg.sequence[++trial.currentToken]===' '?'GAP':exp.cfg.sequence[trial.currentToken])      
        }
      break;
      ...
      case '-':
        if (!cursor.atHome){
          exp.state.next('START');
        }else if (exp.state.expired(exp.cfg.dashDuration*trial.speed)){
          exp.state.next(exp.cfg.sequence[++trial.currentToken]===' '?'GAP':exp.cfg.sequence[++trial.currentToken])
        }
      break;
        // if no keyboard event for total duration
      ...
      case 'U':
        exp.state.once(()=>{
          overlayText.element.innerText = "Reproduce the morse code"
          exp.state.once(()=>console.log(performance.now()))
        })
        if (!cursor.atHome) {
          exp.state.next('START');
        }
        if(exp.state.expired(exp.cfg.responseWaitTime)) {
          exp.state.next('FINISH')
        }
      break;
